...
public class LearningEvaluation extends AbstractMOAObject {
    ...
    public LearningEvaluation(Measurement[] evaluationMeasurements,
            LearningPerformanceEvaluator cpeUp, Learner modelUp,
            LearningPerformanceEvaluator cpeDown, Learner modelDown,
            LearningPerformanceEvaluator cpeMiddle, Learner modelMiddle) {
        List<Measurement> measurementList = new LinkedList<Measurement>();
        if (evaluationMeasurements != null){
        measurementList.addAll(Arrays.asList(evaluationMeasurements));
        }
        Measurement[] measurementsUp = cpeUp.getPerformanceMeasurements();
        Measurement[] measurementsDown = cpeDown.getPerformanceMeasurements();
        Measurement[] measurementsMiddle = cpeMiddle.getPerformanceMeasurements();
                   
        for (int i = 0; i < measurementsUp.length; i++) {
        	measurementList.add(new Measurement("UP " + measurementsUp[i].getName(), measurementsUp[i].getValue()));
        	measurementList.add(new Measurement("DOWN " + measurementsDown[i].getName(), measurementsDown[i].getValue()));
        	measurementList.add(new Measurement("MIDDLE " + measurementsMiddle[i].getName(), measurementsMiddle[i].getValue()));
