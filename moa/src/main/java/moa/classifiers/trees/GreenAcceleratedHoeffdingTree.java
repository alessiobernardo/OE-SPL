 ...
 * @version $Revision: 7 $
 */
public class GreenAcceleratedHoeffdingTree extends AbstractClassifier implements MultiClassClassifier {
    ...
    // Threshold to deactivate nodes
    public FloatOption deactivateThresholdOption = new FloatOption(
            "deactivateThreshold",
            'v',
            "The fraction threshold to deactivate nodes. Fraction = ((inst_node) / (tot_inst_since_node_creation)/n_active_leaves) ",
            0.01, 0.0, Float.MAX_VALUE);

    // Grow with EFDT
    public FloatOption growFastThresholdOption = new FloatOption(
            "growFastThreshold",
            'f',
            "The fraction threshold to grow fast with the EFDT criteria. Fraction = ((inst_node) / (tot_inst_since_node_creation)/n_active_leaves) ",
            2, 0.0, Float.MAX_VALUE);

//    public IntOption greenCheckPeriodOption = new IntOption(
//            "greenCheckPeriod",
//            'k',
//            "How many instances between checking for deactivating nodes or growing fast",
//            100000,0, Integer.MAX_VALUE);
    ...
    public static class Node extends AbstractMOAObject {
        ...
        // How many instances have a specific node observed
        protected int instancesAtNodeCount;

        // Grow with the EFDT on those most visited nodes
        protected boolean growFastOption;

        protected double instancesObservedAtNodeCreation;

        public Node(double[] classObservations) {
            this.observedClassDistribution = new DoubleVector(classObservations);
            // Initialize counter
            this.instancesAtNodeCount = 0;
            this.growFastOption = false;
//            this.instancesObservedAtNodeCreation = this.
        }
        ...
        public double[] getClassVotes(Instance inst, GreenAcceleratedHoeffdingTree ht) {
            return this.observedClassDistribution.getArrayCopy();
        }

        // Instances counter at node
        public int getInstancesAtNodeCount(){ return this.instancesAtNodeCount; }

        public void setInstanceAtNodeCount(int instances){ this.instancesAtNodeCount = instances; }

        public double getInstancesObservedAtNodeCreation() { return this.instancesObservedAtNodeCreation; }

        public void setInstancesObservedAtNodeCreation(double instancesObservedAtNodeCreation) {
            this.instancesObservedAtNodeCreation = instancesObservedAtNodeCreation;
        }
        ...
        public void describeSubtree(GreenAcceleratedHoeffdingTree ht, StringBuilder out,
                                    int indent) {
    ...
    public static abstract class LearningNode extends Node {
        ...
        public abstract void learnFromInstance(Instance inst, GreenAcceleratedHoeffdingTree ht);
    }
    ...
    public static class InactiveLearningNode extends LearningNode {
        ...
        @Override
        public void learnFromInstance(Instance inst, GreenAcceleratedHoeffdingTree ht) {
    ...
    public static class ActiveLearningNode extends LearningNode {
        ...
        public void learnFromInstance(Instance inst, GreenAcceleratedHoeffdingTree ht) {
            if (this.isInitialized == false) {
                ...
                // Initialize the counter of instances at that node
//                setInstanceAtNodeCount(0);
            }
            this.observedClassDistribution.addToValue((int) inst.classValue(),
                    inst.weight());
        ...
        public AttributeSplitSuggestion[] getBestSplitSuggestions(
                SplitCriterion criterion, GreenAcceleratedHoeffdingTree ht) {
    ...
    protected int activateLeafNodeCount;


    protected int growFastCount;
    ...
    protected double majorityClass;
...
//            this.treeRoot.instancesObservedAtNodeCreation = this.trainingWeightSeenByModel;
        }
        ...
        if (leafNode == null) {
            ...
            leafNode.setInstancesObservedAtNodeCreation(this.trainingWeightSeenByModel);
        ...
        if (leafNode instanceof LearningNode) {
            ...
            // Increment the count
            learningNode.setInstanceAtNodeCount(learningNode.getInstancesAtNodeCount()+1);
    ...
    public void greenify (Node leafNode, FoundNode foundNode) {
        //Check instance limit
        if (!(leafNode instanceof InactiveLearningNode) &&
                (this.treeRoot != null) && (leafNode.growFastOption == false) ) {

            double fraction = leafNode.getInstancesAtNodeCount() /
                    ((this.trainingWeightSeenByModel -
                            leafNode.getInstancesObservedAtNodeCreation()) /
                            (this.activeLeafNodeCount));
            if (fraction == 0) {
                System.out.println("Fraction: " + fraction);
                System.out.println("Instances at node count: " + (leafNode).getInstancesAtNodeCount() +
                        " trainingWeightSeenByModel: " + this.trainingWeightSeenByModel +
                        " Instances at Node Creation: " + (leafNode).getInstancesObservedAtNodeCreation() +
                        " LeafNodeCount: " + this.activeLeafNodeCount);
            }

            if ((fraction <
                    this.deactivateThresholdOption.getValue()) &&
                    (leafNode.getInstancesObservedAtNodeCreation() > 0)
                    && this.trainingWeightSeenByModel > 100000) {
                deactivateLearningNode((ActiveLearningNode)
                        leafNode, foundNode.parent, foundNode.parentBranch);
                if (leafNode.growFastOption == true) {
                    growFastCount--;
                }
            } else if (fraction >
                    this.growFastThresholdOption.getValue()) {
                leafNode.growFastOption = true;
                growFastCount++;
            }
        }
    }
    // Deactivate nodes if:
//(instance_node_counter/(total_instances/#nodes))<0.01
    @Override
    public double[] getVotesForInstance(Instance inst) {
//        System.out.println(this.trainingWeightSeenByModel);
        if (this.treeRoot != null) {
            ...
            if (leafNode == null) {
                ...
                leafNode.setInstanceAtNodeCount(leafNode.getInstancesAtNodeCount() + 1);
                greenify(leafNode, foundNode);
            }
    ...
    // Deactivate nodes if: (instance_node_counter/(total_instances/#nodes))<0.01
//    @Override
    /*public double[] getVotesForInstance(Instance inst) {
        if (this.treeRoot != null) {
            FoundNode foundNode = this.treeRoot.filterInstanceToLeaf(inst,
            ...
            //Check instance limit
            if (!(leafNode instanceof InactiveLearningNode) && this.treeRoot != null ){
                double fraction = leafNode.getInstancesAtNodeCount() /
                        ((this.trainingWeightSeenByModel-leafNode.getInstancesObservedAtNodeCreation()) /
                        (this.activeLeafNodeCount));
                if (( fraction < this.deactivateThresholdOption.getValue()) && (leafNode.getInstancesObservedAtNodeCreation()>0)
                        && this.trainingWeightSeenByModel>100000){

//                    System.out.println("Deactivate node with fraction:  "+ fraction +
//                            " instance_node: "+leafNode.getInstancesAtNodeCount() + " n_leaves: "+
//                            this.activeLeafNodeCount + " since_creation: "+
//                            (this.trainingWeightSeenByModel-leafNode.getInstancesObservedAtNodeCreation()));
System.out.println(" After Fraction: " +leafNode.getInstancesAtNodeCount());
                    deactivateLearningNode((ActiveLearningNode) leafNode, foundNode.parent, foundNode.parentBranch);
                }else if (fraction > this.growFastThresholdOption.getValue()){
                    leafNode.growFastOption = true;
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        return new Measurement[]{
                    ...
                    this.activeLeafNodeCount),
                    new Measurement("inactive leaves",this.inactiveLeafNodeCount),
                    new Measurement("growFast nodes",this.growFastCount),
                    new Measurement("majority", this.majorityClass),
    ...
    // Introducing EFDT in nodes where fraction > growFastThresholdOption
        ...
        if (!node.observedClassDistributionIsPure()) {
            ...
            if (bestSplitSuggestions.length < 2) {
                ...
                if (node.growFastOption == false) {
                    ...
                    // EFDT. Note: EFDT calculates it based on the number of splits
//                    System.out.println("EFDT on node: "+node.hashCode());
                    if (bestSuggestion.splitTest == null || bestSuggestion.merit < 1e-10){
                        shouldSplit = false;
                    } else {
                        // Null attribute
                        AttributeSplitSuggestion secondBestSuggestion = bestSplitSuggestions[0];
                        if (((bestSuggestion.merit - secondBestSuggestion.merit) > hoeffdingBound)
                                || (hoeffdingBound < this.tieThresholdOption.getValue())) {
                            shouldSplit = true;
                        }
            ...
            if (shouldSplit) {
                ...
                if (splitDecision.splitTest == null) {
                    ...
                    for (int i = 0; i < splitDecision.numSplits(); i++) {
                        ...
                        // Add the parents counter
                        // INHERIT
//                        newChild.setInstanceAtNodeCount(node.getInstancesAtNodeCount());
                        newChild.setInstancesObservedAtNodeCreation(this.trainingWeightSeenByModel);
                        // Inherit this. Test with both
//                        newChild.growFastOption = node.growFastOption;
                        newSplit.setChild(i, newChild);
//                        if (newChild.growFastOption == true) {
//                            growFastCount++;
//                        }
//                        System.out.println("CHILD weight: " + newChild.getInstancesObservedAtNodeCreation());
                    }
                    this.activeLeafNodeCount--;
            ...
            for (int i = cutoff; i < learningNodes.length; i++) {
                if (learningNodes[i].node instanceof InactiveLearningNode) {
                    activateLearningNode(
                            ...
                            learningNodes[i].parent,
                            learningNodes[i].parentBranch);
                    this.activateLeafNodeCount++;
//                    System.out.println("Activate: "+this.activeLeafNodeCount);
                }
            }
        }
    }
